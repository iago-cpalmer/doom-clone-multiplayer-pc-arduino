...
//********************************************
// Serialize and deserialize floats methods
//********************************************
...
//********************************************
// Serialize and deserialize longs methods
//********************************************
void serializeLongReversed(byte* bytes, long x, int startIndex) {

  longUnion.longValue = x;
  for (int i = 0; i < sizeof(long); i++) {
    bytes[i + startIndex] = longUnion.byteArray[sizeof(long)-1-i];
  }
}
...
void serializeEntity(byte* bytes, Entity* x, int startIndex) {
  serializeLongReversed(bytes, x->uid, startIndex);
...
Entity* deserializeEntity(byte* bytes, int startIndex) {
  ...
  entity->uid = deserializeLong(bytes, startIndex);
